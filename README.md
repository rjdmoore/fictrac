...
1. Download and install required build tools and dependencies:
    ...
    2. Linux (Ubuntu) only:
        1. Run the following from terminal to install necessary build tools and dependencies: ```[Linux] sudo apt-get install gcc g++ git cmake curl unzip tar yasm pkg-config libgtk2.0-dev``` <!---libavformat-dev libavcodec-dev libavresample-dev libswscale-dev-->
...
[Linux] ./vcpkg install opencv[ffmpeg]:x64-linux nlopt:x64-linux boost-asio:x64-linux ffmpeg[x264]:x64-linux
```
